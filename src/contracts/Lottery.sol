...
interface ILottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in METIS per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * msg.value: amount to inject in base token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInMetis: price of a ticket in METIS
 ...
 * randomness provided by SCRT Network.
 */
contract lottery is ReentrancyGuard, ILottery, Ownable {
    ...
    uint256 public maxPriceTicket = 1 ether;
    uint256 public minPriceTicket = 0.00001 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInMetis;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5% 
        uint256[6] MetisPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInMetis;
        uint32 finalNumber;
    }
    ...
    event NewRandomizer(address indexed randomizer);
     ...
     * @param _randomizerAddress: address of the Randomizer contract used to work with SCRT.network's VRF
     */
        ...
        nonReentrant
    {
        ...
        // Calculate number of METIS to this contract
        uint256 amountMetisToPay = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInMetis,
            _ticketNumbers.length
        );

        require(msg.value >= amountMetisToPay, "Insufficient Ticket Fee");

        if (amountMetisToPay > msg.value) {
            address(msg.sender).call{value: msg.value - amountMetisToPay};
        }

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInMetis += amountMetisToPay;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInMetisToTransfer
        uint256 rewardInMetisToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInMetisToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        (bool success, ) = address(msg.sender).call{
            value: rewardInMetisToTransfer
        }("");
        require(success, "Failed to send rewardInMetisToTransfer");

        emit TicketsClaim(
            msg.sender,
            rewardInMetisToTransfer,
            _lotteryId,
            _ticketIds.length
        );
    }
     ...
     * @notice Close lottery
     * @param _lotteryId: lottery id
     * @param _callbackGasLimit: gas limit to be used by SecretVRF callback
     * @param amountOfGas: amount of gas paid for SecretVRF callback
     * @dev Callable by operator
    ...
    function closeLottery(
        ...
        randomizer.getRandomNumber(_callbackGasLimit, amountOfGas);
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        require(
            _lotteryId == randomizer.viewLatestLotteryId(),
            "Numbers not drawn"
        );

        // Calculate the finalNumber based on the randomResult generated by SCRT Network's fallback
        uint32 finalNumber = randomizer.viewRandomResult();
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInMetis) *
        ...
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].MetisPerBracket[j] =
                ...
                // A. No METIS to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].MetisPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId]
            .amountCollectedInMetis - amountToShareToWinners);

        // Transfer METIS to treasury address
        (bool success, ) = treasuryAddress.call{
            value: amountToWithdrawToTreasury
        }("");
     ...
     * @param _randomizerAddress: address of the random generator
     */
    function changeRandomizer(
        ...
        // Request a random number from the new randomizer
        IRandomizer(_randomizerAddress).getRandomNumber(
            _callbackGasLimit,
            amountOfGas
        );

        // Calculate the finalNumber based on the randomResult generated by SCRT Network's fallback
        IRandomizer(_randomizerAddress).viewRandomResult();

        randomizer = IRandomizer(_randomizerAddress);

        emit NewRandomizer(_randomizerAddress);
    }
        ...
        onlyOwnerOrInjector
    {
        ...
        _lotteries[_lotteryId].amountCollectedInMetis += msg.value;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInMetis >= minPriceTicket) &&
                (_priceTicketInMetis <= maxPriceTicket),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInMetis: _priceTicketInMetis,
            ...
            amountCollectedInMetis: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    /**
     * @notice Set METIS price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicket: minimum price of a ticket in METIS
     * @param _maxPriceTicket: maximum price of a ticket in METIS
     */
    function setMinAndMaxTicketPriceInMetis(
        uint256 _minPriceTicket,
        uint256 _maxPriceTicket
    ) external onlyOwner {
        require(
            _minPriceTicket <= _maxPriceTicket,
     ...
     * @param _priceTicket price of a ticket (in METIS)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].MetisPerBracket[_bracket];
        } else {
            return 0;
        }
    }
